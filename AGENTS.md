# Agent guide for the workspaces-per-monitor plugin
...
Every entry point receives, from the host: `pluginApi`, `s` (scale), `active`,
`density`, and `widthBudget`.

`pluginApi` exposes:
- `mainInstance`: the live `service/Main.qml` instance every view shares.
- `pluginSettings`: the resolved settings object.
- `pluginDir`: this plugin's folder.
- `stateDir`: `$XDG_STATE_HOME/ryoku/plugins/<id>`.
